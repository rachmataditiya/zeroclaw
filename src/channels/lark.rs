...
impl LarkChannel {
    ...
    async fn listen_ws(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        if write
            .send(WsMsg::Binary(Bytes::from(initial_ping.encode_to_vec())))
            .await
            .is_err()
        {
        ...
        loop {
            tokio::select! {
                ...
                _ = hb_interval.tick() => {
                    ...
                    if write.send(WsMsg::Binary(Bytes::from(ping.encode_to_vec()))).await.is_err() {
                ...
                msg = read.next() => {
                    ...
                    // ACK immediately (Feishu requires within 3 s)
                    {
                        ...
                        let _ = write.send(WsMsg::Binary(Bytes::from(ack.encode_to_vec()))).await;
                    }
