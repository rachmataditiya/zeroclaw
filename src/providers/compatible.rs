...
impl OpenAiCompatibleProvider {
    ...
    /// Some models (e.g. Moonshot kimi-k2.5) only accept temperature=1. Return the value to send.
    fn effective_temperature(&self, model: &str, temperature: f64) -> f64 {
        if self.name == "Moonshot" && model == "kimi-k2.5" {
            1.0
        } else {
            temperature
        }
...
mod tests {
    ...
    #[test]
    fn effective_temperature_moonshot_kimi_k25_forces_one() {
        let p = make_provider("Moonshot", "https://api.moonshot.ai/v1", None);
        assert_eq!(p.effective_temperature("kimi-k2.5", 0.7), 1.0);
        assert_eq!(p.effective_temperature("kimi-k2.5", 0.0), 1.0);
    }

    #[test]
    fn effective_temperature_moonshot_other_model_unchanged() {
        let p = make_provider("Moonshot", "https://api.moonshot.ai/v1", None);
        assert_eq!(p.effective_temperature("kimi-k2-thinking", 0.5), 0.5);
    }

    #[test]
    fn effective_temperature_non_moonshot_unchanged() {
        let p = make_provider("Venice", "https://api.venice.ai", None);
        assert_eq!(p.effective_temperature("kimi-k2.5", 0.7), 0.7);
    }
