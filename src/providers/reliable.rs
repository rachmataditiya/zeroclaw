...
mod tests {
    ...
    // ── Tests for chat() override ──

    #[tokio::test]
    async fn reliable_provider_chat_delegates() {
        let calls = Arc::new(AtomicUsize::new(0));
        let provider = ReliableProvider::new(
            vec![(
                "primary".into(),
        ...
        assert_eq!(result.text_or_empty(), "chat_ok");
    }

    #[tokio::test]
    async fn reliable_provider_chat_retries() {
        let calls = Arc::new(AtomicUsize::new(0));
        let provider = ReliableProvider::new(
            vec![(
                "primary".into(),
    ...
    #[tokio::test]
    async fn reliable_provider_chat_failover() {
        let primary_calls = Arc::new(AtomicUsize::new(0));
        let fallback_calls = Arc::new(AtomicUsize::new(0));
